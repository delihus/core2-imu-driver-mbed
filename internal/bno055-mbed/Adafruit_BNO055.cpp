...
#define delay(x) ThisThread::sleep_for(x)
#define millis() Kernel::get_ms_count()
 ...
 *          Wire object
 */
Adafruit_BNO055::Adafruit_BNO055(I2C *i2c, int32_t sensorID, uint8_t address)
: _i2c(i2c)
, _sensorID(sensorID)
, _address(address)
{}
...
{
    char data[] = {reg, value};
    return _i2c->write(_address << 1, data, 2, false) == 0 ? true : false;
}
...
uint8_t Adafruit_BNO055::read8(adafruit_bno055_reg_t reg)
{
    ...
    int res = 0;
    char data[] = {reg};
    res += _i2c->write(_address << 1, data, 1, true);
    res += _i2c->read(_address << 1, (char *)&value, 1, false);
                              ...
                              uint8_t len)
{
    ...
    char data[] = {reg};
    res += _i2c->write(_address << 1, data, 1, true);
    res += _i2c->read(_address << 1, (char *)buffer, len, false);
