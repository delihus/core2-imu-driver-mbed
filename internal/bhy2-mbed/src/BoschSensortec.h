...
public:
  ...
  void update();
  /**
   * @brief Configure sensor. @see BHY2::configureSensor()
   *
   * @param config Contains SensorID, SampleRate and latency
   */
  void configureSensor(SensorConfigurationPacket& config);
  /**
   * @brief Set range of the sensor @see BHY2::configureSensor()
   *
  ...
  /**
   * @brief Handle FIFO of data queue. @see BHY2::addSensorData()
   *
   * @param sensorData Data packet from sensor
   */
  void addSensorData(SensorDataPacket &sensorData);
  /**
   * @brief Handle FIFO of data queue for long sensor data @see BHY2::addSensorData()
   *
   * @param sensorData Data packet from sensor
   */
...
private:
  /**
   * @brief The BHY2 class can accces both private and public methods of BoschSensortec
   *
   */
  friend class BHY2;
